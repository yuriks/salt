...
def salt_mm_minion_config(request, salt_factories, salt_mm_master, salt_mm_sub_master):
    return salt_factories.configure_minion(request, 'mm-minion', master_id=salt_mm_master.config['id'])
...
@pytest.fixture(scope='package')
def salt_mm_sub_minion_config(request, salt_factories, salt_mm_master, salt_mm_sub_master):
    return salt_factories.configure_minion(request, 'mm-sub-minion', master_id=salt_mm_sub_master.config['id'])
...
@pytest.fixture(scope='package')
def salt_mm_sub_master(request, salt_factories, salt_mm_master, salt_mm_sub_master_config):
...
def salt_mm_minion(request, salt_factories, salt_mm_master, salt_mm_sub_master):
    return salt_factories.spawn_minion(request, 'mm-minion', master_id=salt_mm_master.config['id'])


@pytest.fixture(scope='package')
def salt_mm_sub_minion(request, salt_factories, salt_mm_master, salt_mm_sub_master):
    return salt_factories.spawn_minion(request, 'mm-sub-minion', master_id=salt_mm_sub_master.config['id'])
...
def bridge_pytest_and_runtests(reap_stray_processes,
    ...
    RUNTIME_VARS.TMP_MM_MINION_CONF_DIR = os.path.dirname(salt_mm_minion.config['conf_file'])
    RUNTIME_VARS.TMP_MM_SUB_CONF_DIR = os.path.dirname(salt_mm_sub_master.config['conf_file'])
    RUNTIME_VARS.TMP_MM_SUB_MINION_CONF_DIR = os.path.dirname(salt_mm_sub_minion.config['conf_file'])
